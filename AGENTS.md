...
- **CI** (`.github/workflows/android.yml`): runs on push/PR to `master` — tests, `assembleDebug`; on push to `master`, creates a GitHub Release with tag `build-<shortSha>` and the debug APK. On pull requests from the same repository (not forks), creates a **prerelease** with tag `pr-<PR#>-<shortSha>` and the debug APK for install testing before merge.
